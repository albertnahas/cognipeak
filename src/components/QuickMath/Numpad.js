...
export const Numpad = ({ setNumber, onOkClickEnter }) => {
    ...
    const onKeyDown = (e) => {
        const key = e.which;
        if (key >= 48 && key <= 57) { // the enter key code or right arrow
    ...
    return (
        <div className="Numpad">
            {[7, 8, 9, 4, 5, 6, 1, 2, 3, 0].map(n => <span key={n} onClick={() => { onNumberClick(n) }}>{n}</span>)}
            <span onClick={onMinusClick} className="minus">{'-'}</span>
            <span onClick={onBackspaceClick} className="backspace">{'<-'}</span>
            <span onClick={onOkClickEnter} className="enter">Enter</span>
        </div>
    )
}
